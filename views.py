...
def getDocuments(request, data_restriction=None):
    ...
    Arguments for formatting response data in json
     ...
     - search_after: Comma-separated list of values for search_after pagination
    Returns
    ...
    def itemFormat(item):
        if '_source' in item:
            ...
            for key in item['_source']:
                if 'topics' not in key:
                    returnItem['_source'][key] = item['_source'][key]
            return returnItem
        else:
            return item

    def jsonFormat(json_response):
        return list(map(itemFormat, json_response['hits']['hits']))

    # Extrahera sort och order från GET-parametrarna
    sort_field = request.GET.get('sort', 'id.keyword')
    sort_order = request.GET.get('order', 'asc')

    # Extrahera search_after från GET-parametrarna
    search_after = request.GET.get('search_after', None)
    if search_after:
        # Dela upp den kommaseparerade strängen till en lista
        search_after_values = search_after.split(',')
    else:
        search_after_values = None

    textField = 'text.raw' if 'search_raw' in request.GET and request.GET['search_raw'].lower() != 'false' else 'text'
    titleField = 'title.raw' if 'search_raw' in request.GET and request.GET['search_raw'].lower() != 'false' else 'title'
    contentsField = 'contents.raw' if 'search_raw' in request.GET and request.GET['search_raw'].lower() != 'false' else 'contents'
    headwordsField = 'headwords.raw' if 'search_raw' in request.GET and request.GET['search_raw'].lower() != 'false' else 'headwords'

    # Bygg upp Elasticsearch-frågan
    query = {
    ...
    # Hantera sortering
    sort_array = []
    if sort_field == 'archive.archive_id_row.keyword':
        # Lägg till sekundärt sorteringsfält för unik ordning
        sort_array.append({sort_field: {'order': sort_order}})
        sort_array.append({'archive.page': {'order': sort_order}})
    else:
        sort_array.append({sort_field: {'order': sort_order}})

    query['sort'] = sort_array

    # Inkludera search_after om det finns
    if search_after_values:
        # För multi-fältsortering, dela upp search_after_values korrekt
        if sort_field == 'archive.archive_id_row.keyword':
            if len(search_after_values) != 2:
                return JsonResponse({'error': 'Invalid search_after parameter'}, status=400)
            try:
                # Anpassa efter datatyperna
                search_after_values = [search_after_values[0], int(search_after_values[1])]
            except ValueError:
                return JsonResponse({'error': 'Invalid search_after values'}, status=400)
        else:
            search_after_values = [search_after_values[0]]  # Anpassa beroende på antalet sorteringsfält
        query['search_after'] = search_after_values
        # Ta bort 'from' när du använder search_after
    else:
        query['from'] = int(request.GET.get('from', 0))

    # Hantera aggregation om den finns
    if 'aggregation' in request.GET:
        ...
        query['aggs'] = {
            'aggresult': {
                aggregation[0]: {
                    'field': aggregation[1],
                    'size': int(aggregation[2]) if len(aggregation) >= 3 else 50
                }
            }
        }

    # Hantera mark_metadata om den finns
    if 'mark_metadata' in request.GET:
    ...
    # Hantera specifik id-sökning om den finns
    if 'id' in request.GET:
    ...
    # Hantera ytterligare sortering om den finns (ytterligare sorteringsfält)
    if 'sort' in request.GET:
        # Detta hanterar redan sorteringen ovan, så det kan eventuellt tas bort eller anpassas
        pass

    # Utför Elasticsearch-frågan
    esQueryResponse = esQuery(request, query, jsonFormat)
    return esQueryResponse
